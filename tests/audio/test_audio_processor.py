...
import pytest
from sanzaru.audio.exceptions import AudioCompressionError, AudioConversionError
...
class TestAudioProcessor:
    ...
    async def test_load_audio_from_path_success(self, tmp_path: Path) -> None:
        ...
        with patch("sanzaru.audio.processor.AudioSegment.from_file", return_value=mock_audio) as mock_from_file:
    ...
    async def test_load_audio_from_path_failure(self, tmp_path: Path) -> None:
        ...
        with patch("sanzaru.audio.processor.AudioSegment.from_file", side_effect=Exception("Invalid audio")):
    ...
    async def test_concatenate_audio_segments_mp3(self) -> None:
        ...
        with (
            patch("sanzaru.audio.processor.AudioSegment.from_mp3") as mock_from_mp3,
            patch("sanzaru.audio.processor.AudioSegment.empty") as mock_empty,
        ):
    ...
    async def test_concatenate_audio_segments_wav(self) -> None:
        ...
        with (
            patch("sanzaru.audio.processor.AudioSegment.from_wav") as mock_from_wav,
            patch("sanzaru.audio.processor.AudioSegment.empty") as mock_empty,
        ):
            mock_empty.return_value = mock_segment
    ...
    async def test_concatenate_audio_segments_failure(self) -> None:
        ...
        with patch("sanzaru.audio.processor.AudioSegment.empty", side_effect=Exception("Concat failed")):
    ...
    async def test_concatenate_audio_segments_empty_list(self) -> None:
        ...
        with patch("sanzaru.audio.processor.AudioSegment.empty") as mock_empty:
