...
SORA_CREATE_VIDEO = """Create a new Sora video generation job. This starts an async job and returns immediately with a video_id.
...
SORA_GET_STATUS = """Check the status and progress of a video generation job.
...
SORA_DOWNLOAD = """Download a completed video to disk.
...
SORA_LIST = """List all video jobs in your OpenAI account with pagination support.
...
SORA_DELETE = """Permanently delete a video from OpenAI's cloud storage.
...
SORA_REMIX = """Create a NEW video by remixing an existing completed video with a different prompt.
...
SORA_LIST_REFERENCES = """Search and list reference images available for video generation.
...
SORA_PREPARE_REFERENCE = """Automatically resize a reference image to match Sora's required dimensions.
...
IMAGE_CREATE = """Generate an image using OpenAI's Responses API and save to reference path.
...
IMAGE_GET_STATUS = """Check status and progress of image generation.
...
IMAGE_DOWNLOAD = """Download a completed generated image to reference path.
