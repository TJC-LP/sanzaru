...
`result.file.path` is the canonical location of a written artifact. Where a result also carries a
bare name (`output_file`, `output_filename`, `filename`), it is the basename of that same path —
including when `-o` renamed the file or it was staged under a temporary name first. The two never
disagree, so `jq -r .result.file.path` and `jq -r .result.output_file` always describe one file.
